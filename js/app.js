    ...
    (() => {
        ...
        function sliders() {
            ...
            if (discountSlider) {
                ...
                if (window.matchMedia("(max-width: 576px)").matches) swiper.destroy();
            }
            const comfortSlider = document.querySelector(".comfort__slider");
            if (comfortSlider) {
                new Swiper(comfortSlider, {
                    modules: [ Navigation, Autoplay, Scrollbar ],
                    speed: 700,
                    slidesPerView: "auto",
                    spaceBetween: 24,
        ...
        function select_select() {
            ...
            if (selects.length) selects.forEach((select => {
                ...
                btnSelect.addEventListener("click", (e => {
                    e.stopPropagation();
                    e.preventDefault();
                    if (select.classList.contains("open")) handleClose(); else {
        ...
        function more() {
            ...
            if (buttons.length) buttons.forEach((btn => {
                ...
                let heightRow = 24;
                if (window.matchMedia("(max-width: 743px)").matches) heightRow = 21;
        ...
        function fancy() {
            Oe.bind('[data-fancybox="certificates"]', {});
            Oe.bind('[data-fancybox="reviews"]', {});
            Oe.bind('[data-fancybox="about"]', {});
            Oe.bind('[data-fancybox="comfort"]', {});
            Oe.bind('[data-fancybox="services-gallery"]', {
                Thumbs: {
                    type: "modern"
                }
            });
        ...
        function date() {
            ...
            if (inputsDate.length) inputsDate.forEach((input => {
                new index_es(input, {
                    onSelect: ({date, formattedDate}) => {
                        if (formattedDate) input.value = formattedDate.split(".").reverse().join("-");
                    }
                });
            }));
        }
        function addClient() {
            ...
            if (parentClients) {
                const btnAdd = createElement("button", [ "btn", "btn-custom", "btn-custom--stroke" ], "btn-add-client");
                ...
                function createClient() {
                    ...
                    const nameClient = createInput("Имя пациента", `name-client-${clientNumber}`, "Екатерина", clientNumber);
                    wrapper.appendChild(nameClient);
                    const patronimycClient = createInput("Отчество пациента", `patronimyc-client-${clientNumber}`, "Максимовна", clientNumber);
                    wrapper.appendChild(patronimycClient);
                    const familyClient = createInput("Фамилия пациента", `family-client-${clientNumber}`, "Кузнецова", clientNumber);
                    wrapper.appendChild(familyClient);
                    const dateClient = createInputDate("Дата рождения", `date-client-${clientNumber}`, "дд.мм.гг", clientNumber);
                    ...
                    if (allClients.length !== 0) {
                        const deleteButton = createElement("button", [ "btn", "btn-custom", "btn-custom--stroke", "_red", "tax__client-btn" ]);
                    ...
                    new index_es(inputDate, {
                        onSelect: ({date, formattedDate}) => {
                            if (formattedDate) inputDate.value = formattedDate.split(".").reverse().join("-");
                        }
                    });
                }
